...
*FIGURE 1b:
	...
	*KEEP RELEVANT VARIABLES
		keep if inlist(Violation,0)
		keep Helping Child Help ViolationHelp 
		
		collapse (mean) mean=Helping (sd) sd=Helping (count) n=Helping, by(Child ViolationHelp)
	...
	*GRAPH
		twoway (bar mean categories if inlist(ViolationHelp,0)) ///
			...
			(scatter mean categories, msym(none) mlab(mean) mlabformat(%9.2f) mlabpos(6) mlabcolor(white) mlabstyle(p1) mlabsize(medsmall)), ///
				xtitle("") ytitle("Helping Rate", size(medium)) ///
				...
				text(.6 1 "n = 101" .6 2 "n = 100" .6 4 "n = 100" .6 5 "n = 100") 
					
		graph export "${results}\Figure1b.pdf", replace		
	restore