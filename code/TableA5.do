...
*ESTIMATES
	eststo tablea5_col1: ///
		qui reg Punishment i.ViolationHelp##i.Child $main_controls if inlist(Violation,1), vce(cluster School)	

	eststo tablea5_col2: ///	
		qui reg Punishment i.ViolationHelp##i.Child i.Child##i.male $main_controls if inlist(Violation,1), vce(cluster School)

*DISPLAY RESULTS		
	esttab tablea5_col1 tablea5_col2, ///
			rename(1.Child "Child" 1.male "Male Target" 1.Child#1.male "Child x Male Target" 1.hugo "Male Actor" 1.Morning "Morning" 1.witness "Witness" 1.rich "Rich area" 1.Rain "Rain" 1.Hot "Hot" _cons "Constant") ///	
			keep("Child" "Male Target" "Child x Male Target" "Male Actor" "Morning" "Witness" "Rich area" "Rain" "Hot" "Constant") ///
			order("Child" "Male Target" "Child x Male Target" "Male Actor" "Morning" "Witness" "Rich area" "Rain" "Hot" "Constant") ///
...
*EXPORT RESULTS		
	...
	using "${results}\TableA5.tex", ///
			...
			postfoot("\hline \\ \end{tabular} \\ \footnotesize \textit{Notes:} Standard errors in parentheses are clustered at the school level. One observation is dropped due to missing data on the target's gender. * p < 0.10 ** p < 0.05 *** p < 0.01. \normalsize") ///
			replace